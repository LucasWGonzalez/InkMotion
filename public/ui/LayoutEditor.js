import MasterSheetGenerator, { DEFAULT_LAYOUT, normalizeLayout } from '../core/MasterSheetGenerator.js?v=5';

const STORAGE_KEY = 'inkmotion-layout-v5';
...
function ensureUi() {
  ...
  if (!document.querySelector('link[href*="/css/layout-editor.css"]')) {
    const link = document.createElement('link'); link.rel = 'stylesheet'; link.href = '/css/layout-editor.css?v=5'; document.head.append(link);
  }
  const section = document.createElement('section');
  section.id = 'layout-studio'; section.className = 'card layout-studio';
  section.innerHTML = `
    <div class="layout-studio-head flow-section-heading"><span class="flow-step">02</span><div><span class="flow-kicker">PASO 2 · DISEÑO</span><h3>Elegí cómo querés imprimirla</h3><p>La vista previa muestra la misma composición que tendrá el PDF y utilizará el reconocimiento WebAR.</p></div><button id="layout-reset" class="text-btn layout-reset" type="button">Restablecer</button></div>
    <div class="layout-mode-switch" role="group" aria-label="Modo de diseño"><button id="layout-mode-classic" class="layout-mode-card is-active" type="button"><strong>Lámina InkMotion</strong><span>Recomendada · máxima compatibilidad</span></button><button id="layout-mode-custom" class="layout-mode-card" type="button"><strong>Personalizar lámina</strong><span>Integra QR y marco a la pieza</span></button></div>
    <div class="layout-studio-grid">
      <div class="layout-preview-shell"><div id="layout-preview-stage" class="layout-preview-stage"><div id="layout-empty" class="layout-empty"><strong>Subí una imagen para ver el diseño final</strong><span>La vista previa será la misma composición que se imprimirá y utilizará para el reconocimiento.</span></div><canvas id="layout-preview-canvas" class="layout-final-canvas" hidden aria-label="Vista previa final de la lámina"></canvas><div id="layout-preview-loading" class="layout-preview-loading" hidden>Actualizando diseño…</div></div><div class="layout-preview-note"><span id="layout-preview-label">Lámina InkMotion</span><span>Así se imprimirá y reconocerá</span></div></div>
      <div id="layout-classic-info" class="layout-classic-info"><strong>Lámina InkMotion seleccionada</strong><p>Conserva el paspartú, el marco técnico y el QR exterior en una composición validada para impresión y reconocimiento.</p><div class="layout-benefits"><span>✓ QR con área de lectura protegida</span><span>✓ Marco técnico estable</span><span>✓ Máxima compatibilidad WebAR</span></div><button id="layout-start-custom" class="btn btn-secondary" type="button">Personalizar lámina</button></div>
      <div id="layout-controls" class="layout-controls" hidden>
        <section class="layout-control-group"><div class="layout-group-head"><div><span class="layout-step">1</span><h4>Código QR</h4></div><small>InkMotion conserva automáticamente el fondo blanco necesario para escanearlo.</small></div>
        ...
        <section class="layout-control-group"><div class="layout-group-head"><div><span class="layout-step">2</span><h4>Marco</h4></div><small>La capa técnica AR se conserva siempre</small></div>
          <label class="layout-field-label">Ubicación del marco</label><div class="layout-choice-row two" role="group" aria-label="Ubicación del marco"><button type="button" data-frame-placement="outside"><strong>Exterior</strong><small>Deja margen alrededor</small></button><button type="button" data-frame-placement="inside"><strong>Sobre la obra</strong><small>El borde entra en la imagen</small></button></div>
          <label class="layout-field-label">Estilo</label><div class="layout-choice-row" role="group" aria-label="Estilo del marco"><button type="button" data-frame-style="technical"><strong>Técnico</strong><small>Esquinas marcadas</small></button><button type="button" data-frame-style="minimal"><strong>Minimal</strong><small>Línea continua</small></button><button type="button" data-frame-style="integrated"><strong>Segmentado</strong><small>Línea discontinua</small></button></div>
...
function syncUi() {
  ...
  el('layout-preview-label').textContent = custom ? 'Diseño personalizado' : 'Lámina InkMotion';
  if (!custom) return;
  mark('data-qr-placement', layout.qr.placement); mark('data-qr-position', layout.qr.position); mark('data-qr-size', layout.qr.size);
...
function updateQuality() {
  ...
  if (layout.qr.placement === 'inside' && layout.qr.position === 'center') { state = 'warning'; title = 'Compatibilidad WebAR: Buena'; message = 'El QR central puede tapar contenido importante de la obra.'; }
  else if (layout.frame.colorMode === 'adaptive' || layout.frame.colorMode === 'custom') message = 'El marco visual cambia de color, pero los anclajes técnicos mantienen alto contraste.';
...
function commitChange(mutator) { if (!isCustom()) setCustomDefaults(); mutator(); layout.qr.style = 'protected'; layout = normalizeLayout(layout); storeLayout(); syncUi(); schedulePreview(); }
